...
a}),m.a=c?m.a:d;l||(d.__class__=d.__static__=b);for(e=0;e<D;e++)B[e]=A(B[e],E,b),r(B[e].call(q,m),function(h,a){d[h]===y&&(d[h]=a)},1);if(!c&&0!==k)if(F){if(G){if(a===q){a="StaticClass"+N+F++;var v=[],s=0,x=["Class "+a];r(d,function(h,a){var f=h.indexOf("$")===0?1:h.indexOf("get ")===0?2:h.indexOf("set ")===0?3:h==="toString"?4:a&&typeof a==="object"&&(a.set||a.get)?5:0;if(f){var b=f===4?h:(s=1)&&f===1?h.substring(1):f===5?h:h.split(" ").pop();k[h]=a;f!==3&&x.push("Public "+(f===4?"Default ":"")+
...
b+"](val)",f)}if(!f){if(v.length===50){x.push("Public ["+v.join("],[")+"]");v.length=0}v[v.length]=h}},1);s?(x.push("Public ["+v.join("],[")+"]","Private [(accessors)]","Private Sub Class_Initialize()","Set [(accessors)]="+a+"FactoryJS()","End Sub","End Class","Function "+a+"Factory()","Set "+a+"Factory=New "+a,"End Function"),t[a+"FactoryJS"]=function(){return g},execVBScript(x.join("\n"))):(k=0,a=u);v=x=u}a&&(g={},r(k,function(a){g[a]=d[a]}),m.a=t[a+"Factory"](),r(d,function(a,b){if(!k.hasOwnProperty(a))if((!w||
l)&&typeof b==="function"){m.a[a]=function(){return b.apply(this===d||this==t?m.a:this,arguments)};m.a[a].toString=function(){return b.toString()}}else m.a[a]=b},1),d=m.a)}}else if(r(n?d:k,function(a,b){var f=a.indexOf("$")===0?1:a.indexOf("get ")===0?2:a.indexOf("set ")===0?3:b&&typeof b==="object"&&(b.set||b.get)?5:0;if(f){if(n){k[a]=b;g++}b=d[a];H.call(d,a)&&delete d[a];var c=f===1?a.substring(1):f===5?a:a.split(" ").pop(),e={enumerable:1,configurable:1,set:u,get:u};if(f!==3)e.get=function(){return((f===
